...
#include <include/dump_main_cmdline.hpp>
#include <include/fstream_default.hpp>
  ...
  if(args.column_flag) {
    ...
    while(it.next()) {
      //if(it.get_val() < lower_count || it.get_val() > upper_count)
        //continue;
      //out << it.get_dna_str() << spacer << std::hex<< it.get_val() << "\n";
      uint64_t value = it.get_val();
      uint32_t taxid = value & 0xFFFFFFFF;
	
      uint16_t in_out = value >> 32 ;
      out << it.get_dna_str() << spacer;
      for (int i = 0; i < 4; ++i) {
      	if ((in_out>>(5+i)) & 0x1) out<<dco[i]<<',';
      }
      out<<spacer;
      for (int i = 0; i < 4; ++i) {
      	if ((in_out>>(i)) & 0x1) out<<dco[i]<<',';
      }
      out<<spacer<< taxid << "\n";
    }
  } else {
