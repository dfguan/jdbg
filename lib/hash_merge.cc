...
#include <include/hash_merge_cmdline.hpp>
...
int merge_main(int argc, char *argv[])
{
  ...
  evo_tree           _evo_tree(args.evo_tree_path.c_str());
  ...
  while(heap.is_not_empty()) {
    ...
    do {
    	uint64_t taxid = head->val & 0XFFFFFFFF;
       uint64_t in_out_degree = head->val & 0XFFF00000000;
       i_val = (i_val & 0XFFF00000000) | in_out_degree | _evo_tree.LCA(i_val & 0XFFFFFFFF, taxid);       
      heap.pop();
      if(head->it->next())
        heap.push(*head->it);
      head = heap.head();
	...
	//if (key == 17)	
    //fprintf(stderr, "%lu\n", i_val &0XFFFFFFFF);
    while(!info.buffers[buf_id].writer.append(key, i_val)) {
      info.cond.lock();
      info.buffers[buf_id].full = true;
      buf_id = (buf_id + 1) % info.nb_buffers;
      while(info.buffers[buf_id].full) { waiting++; info.cond.wait(); }
