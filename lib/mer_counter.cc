...
#include <include/mer_counting.hpp>
#include <include/locks_pthread.hpp>
#include <include/thread_exec.hpp>
#include <include/square_binary_matrix.hpp>
#include <include/count_main_cmdline.hpp>
#include <include/noop_dumper.hpp>
//#include <jellyfish/evo_tree.hpp>
...
class mer_dbg_lca_hash : public mer_counting<jellyfish::parse_dna, inv_hash_t> {
public:
   evo_tree *_evo_tree;
   mer_dbg_lca_hash(const std::vector<const char *> &files, std::vector<uint32_t> &taxons,
                          count_args &_args) :
    mer_counting<jellyfish::parse_dna, inv_hash_t>(_args)
  {

    parser = new jellyfish::parse_dna(files.begin(), files.end(), taxons.begin(), taxons.end(),
                                      args->mer_len_arg, args->buffers_arg,
                                      args->buffer_size_arg);
    _evo_tree = new evo_tree(args->evo_tree_path.c_str());

    ary = new inv_hash_t::storage_t(args->size_arg, 2*args->mer_len_arg,
                                    args->counter_len_arg, 
                                    args->reprobes_arg, 
                                    jellyfish::quadratic_reprobes, _evo_tree);
    if(args->matrix_given) {
      std::ifstream fd;
      fd.exceptions(std::ifstream::eofbit|std::ifstream::failbit|std::ifstream::badbit);
      fd.open(args->matrix_arg.c_str());
...
int count_main(int argc, char *argv[])
{
  ...
  //if (args.evo_tree_path_given) {
 	counter = new mer_dbg_lca_hash(args.file_arg, args.taxons, args); 
  //} else {
	  //if(args.quake_flag) {
