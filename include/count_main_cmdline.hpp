...
public:
  ...
  count_args() :
    ...
    evo_tree_path("evo.map"),evo_tree_path_given(true),
    output_arg("mer_counts"), output_given(false),
    O_flag(false),
    counter_len_arg(10), counter_len_given(false),
    out_counter_len_arg(6), out_counter_len_given(false),
  ...
  count_args(int argc, char* argv[]) :
    ...
    O_flag(false),
    counter_len_arg(10), counter_len_given(false),
    out_counter_len_arg(6), out_counter_len_given(false),//10 bits + 32bits
    both_strands_flag(false),
    reprobes_arg(62), reprobes_given(false),
    raw_flag(false),
    both_flag(false),
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "hVm:s:t:T:o:Oc:Cp:rqL:U:wu";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'q':
        ...
        break;
      case 'T':
	evo_tree_path_given = true;
	evo_tree_path.assign(optarg);
	break;
	...
	const char *path_start = file_names.c_str();	
	const char *path_end = path_start + file_names.size() - 1;
	
	file_arg.push_back(path_start);
	for (; path_start < path_end; ++path_start) {
		if ( 0 == *path_start ) {
			file_arg.push_back(path_start + 1);	
		}
	}
...
#define count_args_HELP "Count k-mers or qmers in fasta or fastq files\n\n" \
  ...
  " -T, --tree-path=string                   File path of phylogentic tree (evo.map)\n" \
