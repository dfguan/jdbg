...
#include <iostream>
#include <vector>
#include <include/double_fifo_input.hpp>
#include <include/atomic_gcc.hpp>
#include <include/misc.hpp>
#include <include/sequence_parser.hpp>
...
namespace jellyfish {
  class parse_dna : public double_fifo_input<sequence_parser::sequence_t> {
    ...
    std::vector<uint32_t>::iterator current_tid;
    bool                        have_seam;
    char                       *seam;
  ...
  public:
    ...
    template<typename T>
    parse_dna(T _files_start, T _files_end, std::vector<uint32_t>::iterator _tid_begin, std::vector<uint32_t>::iterator _tid_end, uint_t _mer_len, 
              unsigned int nb_buffers, size_t _buffer_size); 
	...
	while((sequence = parser->next())) {
          const uint32_t      tid = sequence->tid;
	  const char         *start = sequence->start + 1;
          const char * const  end   = sequence->end - 1;
	  uint64_t i_val;
	  uint64_t r_i_val;
	  //uint64_t in_out_degree = 0;
	  //for(char *estart = sequence->start; estart < sequence->end; ++estart) {
		  //fprintf(stderr,"%c", *estart);
	  //}
	 //fprintf(stderr,"\n"); 
	  while(start < end) {
	      ...
	      if(++cmlen >= mer_len) {
                cmlen  = mer_len;
	       	//initiate value include in_out_degree and taxid 
		char ic = *(start - mer_len - 1);
		char nic =*(start); 
		i_val = (in_out_degree_code[ic]<<5) | in_out_degree_code[nic]; //
	      	r_i_val = (in_out_degree_code[rev_com_code[nic]]<<5)| in_out_degree_code[rev_com_code[ic]];
	      	
		//i_val = ( i_val << 32 ) | tid; 
		      //r_i_val = (r_i_val << 32) | tid; 
                
		      //fprintf(stderr,"%lx\t%lx\n", kmer, i_val);
		//if (rkmer == 0 || kmer == 0) {
			//fprintf(stderr,"%d\t%d\n", ic, nic);	
			//for (int z =0; z < mer_len+2; ++z) {
				//fprintf(stderr, "%c", *(start - mer_len - 1 + z));	
			//}
			//fprintf(stderr, "\t%lx\t%lx", r_i_val, i_val);
			//fprintf(stderr,"\n");
		...
		//if (kmer == 17 || rkmer==17)
			//fprintf(stderr, "%u\n", tid);	
                  ...
                  counter->add(kmer < rkmer ? kmer : rkmer, kmer < rkmer ? i_val: r_i_val,tid, &oval);
		}
                  //counter->add(kmer < rkmer ? kmer : rkmer, kmer < rkmer ? i_val: r_i_val, &oval);
                else
                  //counter->add(kmer, i_val, &oval);
                  counter->add(kmer, i_val,tid, &oval);
...
{
  seam        = new char[mer_len + 1];
  memset(seam, 'A', mer_len + 1);
...
template<typename T>
jellyfish::parse_dna::parse_dna(T _files_start, T _files_end, std::vector<uint32_t>::iterator _taxid_start, std::vector<uint32_t>::iterator _taxid_end, uint_t _mer_len,
            unsigned int nb_buffers, size_t _buffer_size) :
  double_fifo_input<sequence_parser::sequence_t>(nb_buffers), mer_len(_mer_len), 
  buffer_size(allocators::mmap::round_to_page(_buffer_size)),
  files(_files_start, _files_end), current_file(files.begin()),tids(_taxid_start, _taxid_end), current_tid(_taxid_start), 
  have_seam(false), buffer_data(buffer_size * nb_buffers), canonical(false)
{
  seam        = new char[mer_len + 1];
  memset(seam, 'A', mer_len + 1);
