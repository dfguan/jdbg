...
#include <include/token_ring.hpp>
...
namespace jellyfish {
  ...
  void sorted_dumper<storage_t,atomic_t>::_dump() {
	...
	ary->zero_lca_blocks(0, nb_blocks); 
    update_stats();
    _out.close();
    dump_mutex.unlock();
  }
  ...
  void sorted_dumper<storage_t,atomic_t>::dump_to_file(int id) {
    ...
    for(i = id; i * nb_records < ary->get_size(); i += threads) {
      ...
      my_info->heap.fill(it,1);
      //my_info->heap.fill(it);

      while(it.next(1)) {
        typename oheap_t::const_item_t item = my_info->heap.head();
        //if(item->val >= lower_count && item->val <= upper_count)
	  my_info->writer.append(item->key, (item->val << 32 )| item->i_taxid);
	  //std::cerr<<std::hex<<item->key<<"\t"<<std::hex<<item->val<<"\t"<<item->i_taxid<<std::endl;
	  my_info->heap.pop();
      ...
      if(i > 0) {
        ...
        ary->zero_lca_blocks(i * nb_blocks, nb_blocks);
      }
    }
