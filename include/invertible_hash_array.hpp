...
#include <include/square_binary_matrix.hpp>
#include <include/storage.hpp>
#include <include/offsets_key_value.hpp>
#include <include/parse_dna.hpp>
#include <include/misc.hpp>
#include <include/simple_circular_buffer.hpp>
#include <include/evo_tree.hpp>
...
namespace jellyfish {
  namespace invertible_hash {
    ...
    public:
      ...
      array(size_t _size, uint_t _key_len, uint_t _val_len,
            uint_t _reprobe_limit, size_t *_reprobes, evo_tree *_evo_tree_) :
        lsize(ceilLog2(_size)), size(((size_t)1) << lsize),
        size_mask(size - 1),
        reprobe_limit(_reprobe_limit, _reprobes, size), key_len(_key_len),
        key_mask(key_len <= lsize ? 0 : (((word)1) << (key_len - lsize)) - 1),
        ...
        mem_block(div_ceil(size, (size_t)offsets.get_block_len()) * offsets.get_block_word_len() * sizeof(word)),
        data((word *)mem_block.get_ptr()), reprobes(_reprobes),
        lca_mem_blk(size * sizeof(uint32_t)),lca_data((uint32_t *)lca_mem_blk.get_ptr()),
        hash_matrix(key_len), 
        hash_inverse_matrix(hash_matrix.init_random_inverse()), _evo_tree(_evo_tree_)
      {
        if(!data)
          eraise(ErrorAllocation) << "Failed to allocate " 
                                 << (div_ceil(size, (size_t)offsets.get_block_len()) * offsets.get_block_word_len() * sizeof(word))
        ...
        if(!lca_data)
          eraise(ErrorAllocation) << "Failed to allocate " 
                                 ...
                                 << " bytes of memory for taxonomy ids";
      }
	...
	*memlen = blen * offsets.get_block_word_len() * sizeof(word);
	if(*start_ptr + *memlen > end_ptr)
	  *memlen = end_ptr - *start_ptr;
      ...
      void block_to_lca_ptr(const size_t start, const size_t blen,
                        char **start_ptr, size_t *memlen) const {
	*start_ptr = (char *)(lca_data + start * offsets.get_block_len());
	char *end_ptr = (char *)lca_mem_blk.get_ptr() + lca_mem_blk.get_size();

	if(*start_ptr >= end_ptr) {
          *memlen = 0;
	  return;
        }
	// echo taxid was represented by a uint32_t
	*memlen = blen * offsets.get_block_len() * sizeof(uint32_t);
	if(*start_ptr + *memlen > end_ptr)
	  *memlen = end_ptr - *start_ptr;
      }
	...
	memset(start_ptr, '\0', memlen);
      ...
      void zero_lca_blocks(const size_t start, const size_t length) {
	char   *start_ptr;
	size_t  memlen;
	block_to_lca_ptr(start, length, &start_ptr, &memlen);
	memset(start_ptr, '\0', memlen);
      }
      /**
	...
	uint64_t get_end() const { return end_id; }
        ...
        bool next(int i) {
          ...
          while(oid < moid) {
            id = (start_oid + oid++) & mask;
	    //fprintf(stderr,"%lu\n", id);
      //bool get_key_val_full(size_t id, word &key, word &val, uint32_t &_tid, 
                            //bool carry_bit = false) const {
             
	    success = ary->get_key_val_full(id, key, val,i_taxid);
            if(success) {
              hash = (key & ary->key_mask) << ary->lsize;
              uint_t reprobep = (key >> ary->key_off) - 1;
              hash |= (id - (reprobep > 0 ? ary->reprobes[reprobep] : 0)) & ary->size_mask;
        ...
        if(o->key.mask2) {
          ...
          if(key == 0)
            return false;
               ...
               //if (id == 15959) fprintf(stderr,"1sll%lu\n",key); 
	kvw = w + o->val.woff;
        val = ((*kvw) & o->val.mask1) >> o->val.boff;
        if(o->val.mask2)
          val |= ((*(kvw+1)) & o->val.mask2) << o->val.shift;//is a problem here?
	_tid = lca_data[id];
	...
	if(reprobe + buffer.capacity() <= reprobe_limit.val()) {
       ...
       */
      inline bool add(word key, word val, uint32_t _taxid, word *oval=0) {
		//fprintf(stderr,"%lu\n", key);	
	//if (*(data + 15119)) fprintf(stderr,"cid:%d\n", key);
	      uint64_t hash = hash_matrix.times(key);
		return add_lca(hash & size_mask, (hash >> lsize) & key_mask,
		       val, _taxid, oval);
      }
      inline bool add(word key, word val, word *oval = 0) {
		...
		//return add_lca(hash & size_mask, (hash >> lsize) & key_mask,
                       //val, false, oval);
	//else 
	//fprintf(stderr,"uisng the function");
    ...
    private:
      ...
      bool add_lca(size_t id, word key, word val,uint32_t _taxid, word *oval) {
        const offset_t	*ao;
        word		*w;
	...
	//fprintf(stderr,"id: %lu\n", id);	
        ...
        word cary = set_in_out_degree(vw, val, ao->val.boff, ao->val.mask1);
        
	cary >>= ao->val.shift;
	if(cary && ao->val.mask2) { // value split on two words
	  set_in_out_degree(vw+1, cary, 0, ao->val.mask2);
	  //cary >>= ao->val.cshift;
	}
	set_lca(lca_data + id, _taxid);
      ...
      inline bool set_lca(uint32_t *l, uint32_t _taxid) {
	uint32_t now = *l, nw, ow;
	do {
	  ow = now;
	  nw = _evo_tree->LCA(now, _taxid);
          now = atomic.cas(l, ow, nw);
        } while(now != ow);
	return true; 
      }

      inline word set_in_out_degree(word *w, word val, uint_t shift, word mask) {
